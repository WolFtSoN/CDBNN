...
void report_coverage_results(SoftMCPlatform &platform, const std::vector<uint32_t>& pattern, const std::vector<uint32_t>& r_frac_idx, uint32_t bank_id, uint32_t num_iter, std::array<uint32_t, 8192 * 8>& maj_result, vector<uint32_t>& open_row_idx, bool is_stability)
{
...
int main(int argc, char*argv[])
{
  ...
  int coverage_iter_count = 1000;
  std::vector<std::string> pattern_files;
  std::vector<std::array<uint32_t, 8192 * 8>> coverage_result;
  ...
  for (boost::filesystem::directory_entry& entry : boost::filesystem::directory_iterator("./patterns/"))
  {
    ...
    std::array<uint32_t, 8192 * 8> pattern_coverage_result = {0};
    std::vector<uint32_t> patterns;
    parse_file(entry.path().string(), patterns);
    for (int j = 0; j < coverage_iter_count; j++)
    {
      ...
      report_coverage_results(platform, patterns, r_frac_idx, bank_id, num_iter, pattern_coverage_result, open_row_idx, false);
    }
    coverage_result.push_back(pattern_coverage_result);
  }
  ...
  {
    avg_coverage = 0.0;
    num_full_coverage_cells = 0;
    for(int i = 0; i < 8192*8; i++)
    {
      if(coverage_result[j][i] == coverage_iter_count)
      {
        num_full_coverage_cells++;
      }
      avg_coverage += coverage_result[j][i];
    }
    out_file << std::to_string((avg_coverage/(double)coverage_iter_count)/65536.0 * 100.0) << ",";
    out_file << std::to_string((num_full_coverage_cells/65536.0)*100.0) << ",";
  }
  out_file.close();
