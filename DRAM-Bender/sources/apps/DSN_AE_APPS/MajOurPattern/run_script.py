...
for rows in [6]: # [4,8,16,32]
    ...
    for t_12 in t_12_lst:
        for t_23 in t_23_lst:
            sample_csv = f'{apps_path}../../../../experimental_data/{module}/samples_check_MAJ3_comprows.csv'
            ...
            samples_df = samples_df.loc[samples_df['Rfirst'] == 408]
            samples_df = samples_df.loc[samples_df['Rsecond'] == 665]
            ...
            csv_file = f'maj_coverage_check_MAJ3_comprows.csv'
            lst.to_csv(csv_file)
            for majX in [3]: # Instead of checking for [3,5,7,9] we only need to check for 3
                ...
                #     end_time = 0
                pattern_file_name = 'kuku'
                print(f'Open Rows: {rows}, #Sample: {len(samples_df)}, #MAJ: {majX}')
