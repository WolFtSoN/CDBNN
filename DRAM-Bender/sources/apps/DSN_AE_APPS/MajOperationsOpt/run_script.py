...
for rows in [6,8,12,16]: # [4,8,16,32]
    # if temperature == '50':
    #     t_12_lst = [0,1,2,3]
    #     t_23_lst = [0,1,2,3]
    #     # bank_lst = [0,1,2,3]
    ...
    for name_t12 in t_12_lst:
        for name_t23 in t_23_lst:
            sample_csv = f'{apps_path}../../../../experimental_data/{module}/samples5_{rows}_{name_t12}_{name_t23}.csv'
            ...
            csv_file = f'maj_coverage5_{rows}_{name_t12}_{name_t23}.csv'
            lst.to_csv(csv_file)
            for majX in [5]: # Instead of checking for [3,5,7,9] we only need to check for 3
                if (majX > rows):
                    break
