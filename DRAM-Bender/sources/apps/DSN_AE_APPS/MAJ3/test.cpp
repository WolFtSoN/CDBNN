                  ...
                  uint32_t t_12, uint32_t t_23, uint32_t n_frac_times, uint32_t t_frac)
{
  ...
  // Initialize number of iterations: 0 -> LOOP_ITER reg
  program.add_inst(SMC_LI(0, LOOP_ITER));
  program.add_label("ITER_LOOP");
    // Use pre as some kind of initialization again - why?
    program.add_below(PRE(BAR, 0, 0));
    // Run on all rows paticipating in MAJ, fill with given pattern
    for(int i = 0; i < n_open_rows; i++)
    {
      random = rand();
      // This writes a value into a row - understand what this does EXACTLY. (why rand?, row is larger than 32b)
      program.add_below(wrRow_immediate_label(BAR, open_row_idx[i], patterns[i],random));
    }
    // 6 MC sleep cycles ~ 24 quite cycles for memory. Unsure why.
    program.add_inst(SMC_SLEEP(6));
    // PRE as initializaiton - why?
    program.add_below(PRE(BAR, 0, 0));
    // More sleep probably to let precharge complete
    program.add_inst(SMC_SLEEP(6));
    // Loop over frac rows and fill with VDD/2
    for (int i = 0; i < r_frac_idx.size(); i++)
    {
      // repeat frac operation for closer results to VDD/2
    ...
    program.add_below(PRE(BAR, 0, 0));
    program.add_inst(SMC_SLEEP(6));
    // Open multiple rows using ACT - PRE - ACT
    program.add_below(doubleACT(t_12, t_23, r_first, r_second));
    // Give some time to settle, and re-initialize with PRE
    program.add_inst(SMC_SLEEP(6));
    program.add_below(PRE(BAR, 0, 0));
    program.add_inst(SMC_SLEEP(6));

    // Read first row of the opened row after MAJ performed
    program.add_below(rdRow_immediate(BAR, open_row_idx[0]));
    // Again - quite, precharge and quite.
    program.add_inst(all_nops());
    program.add_inst(all_nops());
    program.add_below(PRE(BAR, 0, 0));
    program.add_inst(all_nops());
    program.add_inst(all_nops());
    // Up till here the row contents should have been read into the FPGA buffer

    program.add_inst(SMC_ADDI(LOOP_ITER, 1, LOOP_ITER));
  // repeat loop for num_iters times
...
int main(int argc, char*argv[])
{
  ...
  uint32_t coverage_result[8192*8] {0}; // Why this size?
  ...
  */
  ...
  { 
    // Trick to aggregate results over many tested patterns
    if(coverage_result[j] == num_patterns)
    {
      // this is the number of cells that passed all tests
