...
Program timing_test(uint32_t bank_id)
{
  Program program;
  program.add_below(_init(bank_id));
  srand((unsigned) time(NULL));
  ...
  random = rand();
  program.add_below(wrRow_immediate_label(BAR, 0, ZERO ,random));
  random = rand();
  program.add_below(wrRow_immediate_label(BAR, 1, ONE ,random));
  // PRECHARGE - minimum is 5 DRAM (666MHz) cycles 
  // program.add_inst(SMC_PRE(BAR, 0, 0),SMC_NOP(), SMC_NOP(), SMC_NOP());
  // program.add_inst(SMC_SLEEP(4));
  random = rand();
  program.add_below(rdRow_immediate_label(BAR, 1, random)); // Return ONE
  program.add_inst(SMC_SLEEP(4));
  // program.add_below(PRE(BAR, 0, 0));
  // PRECHARGE -> WAIT tRP (5) -> ACT R0 -> WAIT tRCD (10) -> PRE -> (1) ACT R1 (quickly) -> tRAS (13 - 16 actually)
  program.add_inst(SMC_LI(0, RF_REG));
  program.add_inst(SMC_LI(1, LOOP_COLS));
  program.add_inst(SMC_PRE(BAR, 0, 0), SMC_NOP(), SMC_NOP(),SMC_NOP());
  program.add_inst(SMC_NOP(), SMC_NOP(),SMC_ACT(BAR, 0, RF_REG, 0), SMC_NOP());
  program.add_inst(all_nops());
  program.add_inst(all_nops());
  program.add_inst(SMC_NOP(), SMC_PRE(BAR, 0, 0), SMC_NOP(), SMC_ACT(BAR, 0, LOOP_COLS, 0));
  // program.add_inst(SMC_SLEEP(4)); // 13 ~ 3
  program.add_inst(all_nops()); // read row has intenally 3 INSTRUCTIONS
  random = rand();
  program.add_below(rdRow_immediate_label(BAR, 1, random)); // Return ONE
  // p.add_inst(SMC_NOP(),SMC_NOP(),SMC_ACT(bank_reg, 0, RAR, 0),SMC_NOP());
...
int main(int argc, char*argv[])
{
  ...
  {
    out_file << std::to_string(row[i]) << endl;
  }
